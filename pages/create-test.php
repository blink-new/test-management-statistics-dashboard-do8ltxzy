...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_test'])) {
    ...
    if (empty($title)) {
        ...
        try {
            $testId = createTest($title, $description, $timeLimit, $questionIds);
            header("Location: ?page=tests&created=1");
            exit;
        } catch (Exception $e) {
...
<div class="space-y-6">
    ...
    <div class="flex items-center space-x-4">
        ...
        <div>
            ...
            <p class="text-gray-600 mt-1">Build a new test by selecting questions</p>
        </div>
    </div>
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="title" name="title" required 
                           ...
                           placeholder="Enter test title" value="<?= $_POST['title'] ?? '' ?>">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="time_limit" name="time_limit" min="0" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                           placeholder="0 = No limit" value="<?= $_POST['time_limit'] ?? '0' ?>">
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <textarea id="description" name="description" rows="3" 
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          placeholder="Enter test description"><?= $_POST['description'] ?? '' ?></textarea>
            </div>
        </div>
                        ...
                        <div class="question-item ml-6 p-4 border border-gray-200 rounded-lg" data-group-id="<?= $question['group_id'] ?>">
                            <label class="flex items-start space-x-3 cursor-pointer">
                                <input type="checkbox" name="questions[]" value="<?= $question['id'] ?>" 
                                       ...
                                       <?= in_array($question['id'], $_POST['questions'] ?? []) ? 'checked' : '' ?>>
                                <div class="flex-1">
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" name="create_test" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                Create Test
            </button>
        </div>
    </form>
